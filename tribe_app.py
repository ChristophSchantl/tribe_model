...
tickers_input = st.sidebar.text_input("Tickers (Komma-getrennt)", value="BABA,QBTS,VOW3.DE,INTC")
TICKERS = [t.strip().upper() for t in tickers_input.split(",") if t.strip()]
START_DATE = st.sidebar.date_input("Startdatum", value=pd.to_datetime("2024-01-01"))
END_DATE = st.sidebar.date_input("Enddatum", value=pd.to_datetime(datetime.now().date()))
...
ENTRY_PROB = st.sidebar.slider("Entry Schwelle (P(Signal))", min_value=0.0, max_value=1.0, value=0.63, step=0.01)
EXIT_PROB = st.sidebar.slider("Exit Schwelle (P(Signal))", min_value=0.0, max_value=1.0, value=0.46, step=0.01)
COMMISSION = st.sidebar.number_input("Kommission (pro Trade, Anteil)", min_value=0.0, max_value=0.02, value=0.005, step=0.001, format="%.4f")
INIT_CAP = st.sidebar.number_input("Initiales Kapital (€)", min_value=1000.0, value=10_000.0, step=1000.0, format="%.2f")
